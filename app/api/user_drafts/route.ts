...
export async function GET(req: Request) {
	...
	const draft = await db.collection('user_drafts').findOne({ draftId, userId: session.user.id });
...
export async function POST(req: Request) {
	...
	await db.collection('user_drafts').updateOne(
		{ draftId, userId },
		{ $set: { draftId, userId, userName, department, entries: normalizedEntries, updatedAt: now, createdAt: now } },
		{ upsert: true }
	);
